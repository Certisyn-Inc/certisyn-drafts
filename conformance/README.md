...
| `arp-deterministic-encoding-v0.1.json` | Ours. Eight known-answer rows whose **expected bytes are fixed in the file and derived by nothing at run time**, plus three defective encoders. This is the only class in the tree whose expectations do not come from the implementation under test, and it exists to close the encoder-independence gap the existence-oracle class declared. **Generated** by `runners/build_deterministic_encoding_vectors.py`; never hand-edited. |
...
| `build_deterministic_encoding_vectors.py` | Regenerates `vectors/arp-deterministic-encoding-v0.1.json`. **Needs `cbor2`; the runner does not and must not.** cbor2 corroborates item encodings only. It does not corroborate map key order, because its canonical mode implements RFC 8949 Section 4.2.3 length-first ordering and not the Section 4.2.1 bytewise ordering this document cites — measured, and recorded in the vector file. Two implementations that disagree are not a vote; the RFC decides it. |
| `run_deterministic_encoding_vectors.py` | The eight known-answer rows against the harness encoder, deriving nothing, plus three defective encoders each of which must be caught by the row designed to trip it. The length-first encoder is caught by **DE-06 alone**, which is what establishes that DE-06 is doing work no other row does. |
