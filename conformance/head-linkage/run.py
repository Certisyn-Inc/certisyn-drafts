...
def main(w):
    ...
    cb, cr = versions()
    w("draft-hillier-scitt-arp-03, head linkage under the quorum rule")
    w("what the linkage arrays pin, and what they do not")
    w("the repair measured as holding is the one -04 specifies, at 5.23.2.3")
    w()
    w("environment")
    ...
    w()
    w("   On the relationship to deployed practice. An independent party that")
    w("   signs and serves a bounded, floored, addressable interval of a log's")
    w("   contents from its own origin is the mirror role, specified at C2SP")
    w("   tlog-mirror and deployed. Nothing here claims it. A mirror assumes a")
    w("   Merkle log and carries subtree consistency proofs, and neither is")
    w("   available over a plain hash chain with no skip links and no history")
    w("   tree, which is the case measured above.")
    w()
